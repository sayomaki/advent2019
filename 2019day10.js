...
astro.forEach(ast => {
  ...
  astro.forEach(compare => {
    ...
    // count unique heading (not blocked)
    if (insight.indexOf(dir) > -1) {
      return;
    }
    else {
      insight.push(dir);
  ...
  if (insight.length > maxAst) {
    maxAst = insight.length;
    coord = ast;
  }
});

console.log('Max:', maxAst);
console.log('Coords for Part 2:', coord);
